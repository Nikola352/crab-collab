...
> **Status: original proposal.** Written before implementation and kept unchanged as a
> planning record — it describes intent, not the finished system. Several decisions changed
> along the way (most notably: no Automerge; cell text uses Operational Transformation and
> cell ordering uses fractional indexing). For what was actually built, see the
> [README](../README.md).
...
- **Multiple Kernels**: Single Python kernel per session
- **Multi-Language Support**: Python execution only (no R, Julia, etc.)
